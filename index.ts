...
import type { TransactionData } from 'enft'; // Change the path to 'enft' if you are using the package
...
function getSecretsFromEnvFile(): void {
...
async function getSecretsFromKms(): Promise<void> {
  ...
  // Your code goes here
    try {
        const getSecretValueResponse = response = await client.send(
        ...
        const kmsSecrets: { [key: string]: string } = JSON.parse(secretString);
